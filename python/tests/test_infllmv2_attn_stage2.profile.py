...
DTYPE = torch.float16
TOPK = 32
SPARSITY = TOPK * BLOCK_SIZE / SEQLEN_K
...
if __name__ == "__main__":
    ...
    print("[INFO] q.shape:", q.cpu().numpy().shape)
    print("[INFO] k.shape:", k.cpu().numpy().shape)
    print("[INFO] v.shape:", v.cpu().numpy().shape)

    query_padding_mask = generate_random_padding_mask(SEQLEN_Q, BATCH_SIZE, DEVICE, mode="random")
    key_padding_mask = generate_random_padding_mask(SEQLEN_K, BATCH_SIZE, DEVICE, mode="random")
    print("[INFO] query_padding_mask.shape:", query_padding_mask.cpu().numpy().shape)
    print("[INFO] key_padding_mask.shape:", key_padding_mask.cpu().numpy().shape)
    ...
    print("[INFO] topk_idx.shape:", topk_idx.cpu().numpy().shape)

    # exit()

    base_blockmask = convert_topk_to_base_blockmask(topk_idx, max_seqlen_k, BLOCK_SIZE, DEVICE)
    print("[INFO] base_blockmask.shape:", base_blockmask.cpu().numpy().shape)
    ...
    print("[INFO] mixed_mask.shape:", mixed_mask.cpu().numpy().shape)
    print("[INFO] causal:", CAUSAL)
    ...
    for _ in range(TEST_ITERS_TORCH):
        ...
        torch.mps.empty_cache()
        gc.collect()
        print(f"[INFO] torch impl time: {elapsed_time:.2f} ms")
        time.sleep(0.5) # sleep 0.5 s

    # Explicitly clean up MPS events before exit to avoid destructor errors
    del start_event
    del end_event
    gc.collect()
    ...
    # print(f"[INFO] torch prefill/decode rate: {SEQLEN_Q / np.mean(time_list) * 1000:.2f} tok/s")
    
    # exit()

    out_ref = out_ref.permute(0, 2, 1, 3).contiguous() # (1, 2048, 32, 128) -> (1, 32, 2048, 128)
    print("[INFO] out_ref.shape:", out_ref.detach().cpu().numpy().shape)
    ...
    print("[INFO] q_mlx.shape:", q_mlx.shape)
    print("[INFO] k_mlx.shape:", k_mlx.shape)
    print("[INFO] v_mlx.shape:", v_mlx.shape)
    ...
    print("[INFO] blockmask_uint64.shape", blockmask_uint64.shape)
    ...
    print("[INFO] out_mlx.shape:", out_mlx.shape)
    ...
    # print(f"[INFO] time_list: {time_list}")
    print(f"[INFO] mlx mean time: {np.mean(time_list):.4f} ms")
    print(f"[INFO] mlx std time: {np.std(time_list):.4f} ms")
    print(f"[INFO] mlx min time: {np.min(time_list):.4f} ms")
    print(f"[INFO] mlx max time: {np.max(time_list):.4f} ms")
    # print(f"[INFO] mlx prefill/decode rate: {SEQLEN_Q / np.mean(time_list) * 1000:.4f} tok/s")
    
    diff = mx.abs(out_mlx - out_ref_mlx)
    print("[INFO] diff.shape:",diff.shape)
    print(f"[INFO] diff.max(): {diff.max():.4f}")
    print(f"[INFO] diff.min(): {diff.min():.4f}")
